...
test_that("req_next_multi() produces the request to the next page", {
  resp <- response()
  ...
  out <- req_next_multi(req, parsed = NULL)
...
test_that("req_paginate_offset() can paginate", {
  ...
  resp <- req_perform(req2)
  req3 <- req_next_multi(req2, parsed = NULL)
