...
[`req_body_json()`](https://httr2.r-lib.org/reference/req_body.md) to
add some data encoded as JSON:
...
[`req_body_form()`](https://httr2.r-lib.org/reference/req_body.md) uses
the encoding used when you submit a form from a web browser:
...
#> accept-encoding: deflate, gzip, br, zstd
#> content-length: 246
#> content-type: multipart/form-data; boundary=------------------------ZcgqdxvnJc3J1oe8xkPSTz
#> host: 127.0.0.1:39515
#> user-agent: httr2/1.2.2 r-curl/7.0.0 libcurl/8.5.0
#> 
#> --------------------------ZcgqdxvnJc3J1oe8xkPSTz
...
[`req_body_raw()`](https://httr2.r-lib.org/reference/req_body.md) to add
the data to the body and set the `Content-Type` header.
...
[`resp_raw()`](https://httr2.r-lib.org/reference/resp_raw.md):

``` r
resp |> resp_raw()
#> HTTP/1.1 200 OK
#> Date: Mon, 08 Dec 2025 15:01:02 GMT
#> Content-Type: application/json
#> Content-Length: 407
#> ETag: "de760e6d"
#> 
...
- [`req_cache()`](https://httr2.r-lib.org/reference/req_cache.md) sets
  up a cache so if repeated requests return the same results, you can
...
- [`req_retry()`](https://httr2.r-lib.org/reference/req_retry.md) sets
  up a retry strategy so that if the request either fails or you get a
  transient HTTP error, it’ll automatically retry after a short delay.
