...
#>  $ data  :List of 1
#>   ..$ :List of 3
#>   .. ..$ title      : chr "Odit animi et aut."
#>   .. ..$ description: chr "Facere nam eos at similique non et. Ipsa animi est pariatur. Dignissimos aut voluptatem ut quaerat."
...
#> Date: Mon, 08 Dec 2025 15:01:10 GMT
...
can leave the
[`req_error()`](https://httr2.r-lib.org/reference/req_error.md) default
as is. We’ll have another go later with an API that does provide more
details.
...
#>  $ data  :List of 1
#>   ..$ :List of 3
#>   .. ..$ title      : chr "Qui quae sunt quo eaque fuga."
#>   .. ..$ description: chr "Nemo repellendus possimus illum et expedita sunt saepe id. Id tempora et perferendis dolor repellendus. Possimu"| __truncated__
...
- I’ve used a default of `NULL` for the `seed` argument.
  [`req_url_query()`](https://httr2.r-lib.org/reference/req_url.md) will
  automatically drop `NULL` arguments so this means that no default
...
[`req_url_query()`](https://httr2.r-lib.org/reference/req_url.md) uses
[dynamic dots](https://rlang.r-lib.org/reference/dyn-dots.html), so we
can use `!!!` to convert (e.g.)
...
#>   .. ..$ id       : int 1
#>   .. ..$ firstname: chr "Kiel"
#>   .. ..$ lastname : chr "Tremblay"
#>   .. ..$ email    : chr "magnus.bashirian@yahoo.com"
#>   .. ..$ phone    : chr "+12312574290"
#>   .. ..$ birthday : chr "1968-07-25"
#>   .. ..$ gender   : chr "male"
#>   .. ..$ address  :List of 10
#>   .. .. ..$ id            : int 1
#>   .. .. ..$ street        : chr "549 Grant Shoal Suite 732"
#>   .. .. ..$ streetName    : chr "Wilkinson Motorway"
#>   .. .. ..$ buildingNumber: chr "4867"
#>   .. .. ..$ city          : chr "Hilmafort"
#>   .. .. ..$ zipcode       : chr "52788-7338"
#>   .. .. ..$ country       : chr "Moldova"
#>   .. .. ..$ country_code  : chr "MD"
#>   .. .. ..$ latitude      : num -79.1
#>   .. .. ..$ longitude     : num 55.4
#>   .. ..$ website  : chr "http://carroll.net"
...
#>   <chr>     <chr>        <chr>                         <chr> 
#> 1 Brendan   Pollich      oswaldo.koch@yahoo.com        male  
#> 2 Sheldon   Wisoky       maud.rohan@spinka.org         male  
#> 3 Wilmer    Pfannerstill brett69@brown.com             male  
#> 4 Rocio     Kerluke      mackenzie.pollich@fritsch.biz male  
#> 5 Norris    Schneider    upurdy@heller.info            male
```
...
httr2 provides
[`secret_encrypt()`](https://httr2.r-lib.org/reference/secrets.md) and
[`secret_decrypt()`](https://httr2.r-lib.org/reference/secrets.md) to
scramble secrets so that you can include them in your public source code
without worrying that others can read them. There are three basic steps
to this process:
...
2.  You scramble your secrets with
    ...
    secret_scrambled
    #> [1] "_2gDccvBmYnF8OkLAm2_-dWpZWlTb4ByNCetgrSuKfS0F-GJ5F7mJ0GV5cC9wkk6JQ"
    ```
...
secret_scrambled
#> [1] "-2_uP9y9jj5m1-Nl6Gg3dY5yYg57ojLVW3GPoH9M8pws2aW-hNzCTBeelwaNa2n9wA"
...
- You don’t need to do anything in tests because when
  [`secret_decrypt()`](https://httr2.r-lib.org/reference/secrets.md) is
  run by testthat, it will automatically `skip()` the test if the key
  isn’t available.
...
[`req_dry_run()`](https://httr2.r-lib.org/reference/req_dry_run.md), or
even just print the request object. And indeed, you’ll see that in the
examples below — this is bad practice for a real package, but I think
it’s ok here because the key doesn’t allow you to do anything valuable
...
to [`req_error()`](https://httr2.r-lib.org/reference/req_error.md). This
should be a function that takes a response and returns a character
...
[`req_retry()`](https://httr2.r-lib.org/reference/req_retry.md), which
automatically waits the amount of time the server requests. Instead,
we’ll use
[`req_throttle()`](https://httr2.r-lib.org/reference/req_throttle.md) to
ensure we don’t make more than 10 requests every 60 seconds:
...
[`req_error()`](https://httr2.r-lib.org/reference/req_error.md) and it
will be automatically included in the error when a request fails:
...
[`req_retry()`](https://httr2.r-lib.org/reference/req_retry.md) so httr2
has all the information it needs to handle rate-limiting automatically:
...
[`req_method()`](https://httr2.r-lib.org/reference/req_method.md) to
