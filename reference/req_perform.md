...
After preparing a
[request](https://httr2.r-lib.org/reference/request.md), call
...
- If the HTTP request succeeds, and the status code is ok (e.g. 200), an
  HTTP [response](https://httr2.r-lib.org/reference/response.md).
...
- If you have configured retries with
  ...
  the request fails with a transient problem, `req_perform()` will try
  again after waiting a bit. See
...
[`req_throttle()`](https://httr2.r-lib.org/reference/req_throttle.md) or
[`req_retry()`](https://httr2.r-lib.org/reference/req_retry.md). You can
turn the progress bar off (and just show the total time to wait) by
setting `options(httr2_progress = FALSE)`.
