...
automatically convert HTTP errors (i.e. any 4xx or 5xx status code) into
R errors. Use `req_error()` to either override the defaults, or extract
additional information from the response that would be useful to expose
to the user.
...
designed to succeed if and only if you get a valid HTTP response. There
are two ways a request can fail:
...
[`req_retry()`](https://httr2.r-lib.org/reference/req_retry.md) to
...
last_response()
#> <httr2_response>
#> GET http://127.0.0.1:41151/status/404
