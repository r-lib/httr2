...
If you're wrapping this code into a package, it's considered polite to set a user agent, so that, if your app accidentally does something horribly wrong, the developers of the website can figure out who to reach out to.
...
Secrets are important, because every API (except for very simnple APIs like faker) is going to require that you identify yourself in some way typically with a username and password or an API key.
...
But you almost invariably accumulate more secrets over time, and more people and computers that you need to share it with, so I think spending little time to understand this system and set up it for your package will pay off in the long term.
...
2.  You scramble your secrets with `secret_encrypt()` and store the resulting text directly in the source code of your package:
    ...
    ```{r}
    secret <- secret_encrypt("this is a secret message", key)
    secret
    ```
...
3.  When needed, you descramble the the secret using `secret_decrypt()`:
    ...
    ```{r}
    secret_decrypt(secret, key)
    ```

### Package secrets

You can create an number of encryption keys, but I highly recommend that you create one key per package, which I'll call the **package** key.
...
Now (after you restart R), you'll be able to take advantage of a special `secret_encrypt()` and `secret_decrypt()` feature: the encryption key can be the name of an environment variable.

```{r}
secret <- secret_encrypt("my personal secret", "YOURPACKAGE_KEY")
secret
secret_decrypt(secret, "YOURPACKAGE_KEY")
```

You'll also need to make the key available in your GitHub Actions so your automated tests can use it.
...
Other continuous integration platforms will offer similar ways to make secrets available in environment variables.

### When the package key isn't available

There's an important computer that won't have your package key: CRAN.
That means if you want to share your package on CRAN, you need to make sure that your examples, vignettes, and tests all work without error.
