...
JENNY COMMENT HERE The httr2 details get in your way here, because if you retrieve an unsuccessful HTTP response, you automatically get an error that prevents you from further inspecting the body:
...
Next we'll take a look at the NYTimes [Books API](https://developer.nytimes.com/docs/books-product/1/overview).
It requires a very simple authentication with an API key that's included in every request.
...
httr2 provides the `secret_encrypt()` and `secret_decrypt()` functions that use symmetric cryptography.
...
So start by opening your user-level `.Renviron` (e.g. with `usethis::edit_r_environ()`) and add a line like:
...
```{r}
x_enc <- secret_encrypt("API-key-from-NYTimes-Books", "HTTR2_KEY")
x_enc
secret_decrypt(x_enc, "HTTR2_KEY")
```
...
That means that your auth-requiring tests will automatically be skipped on CRAN.
...
All in all, this means you need to be a little cautious if you need to get help with a reprex; you'll need to manually redact the key.
...
JENNY SARCASTIC AND BITTER COMMENT HERE
...
```{r}
github_error_body <- function(resp) {
...
request("https://api.github.com/gists") %>% 
  ...
  req_headers(Authorization = paste("token", token)) %>% 
  req_error(body = github_error_body) %>% 
  req_perform()
```
...
To [create a gist](https://docs.github.com/en/rest/reference/gists#create-a-gist) we need to send a `POST` request with a body that contains data encoded as JSON.
...
-   `req_oauth_client_credentials()` is used to allow the client to perform actions on its own behalf (instead of on behalf of some other user). JENNY COMMENT HERE
...
-   The request returns a 401 status code.

JENNY COMMENT HERE

If the access token has expired, httr2 will automatically retry automatically refresh before the first request.
...
However, there's no way to protect other R code from using httr2 to access them, so if you do choose to cache tokens, you should inform the user and give them the ability to opt-out.
