...
You'll probably want to wrap this up in a function in your package.
For an example, you might take a look at `example_github_client()`, which I provide in httr2 because I use it in a couple of different OAuth examples.

## Authorization code flow

Once you have a client you need to use it with a **flow** in order to get a token.
OAuth provides a number of different "flows", the most common is the "authorisation code" flow, which is implemented by a pair of functions `oauth_flow_auth_code()` and `req_oauth_auth_code()`.

I'd recommend starting with the `oauth_flow_` function.
This isn't going to be useful in the long run, but it's the fastest way to iterate to ensure that you can get an OAuth token interactively.
For example, you can get a token to access the GitHub API using the example client above with this code:
...
But if you do run it and print out the `token`, you'll see something like this:
...
If it doesn't, you'll need to do a bunch of careful checking and experimentation 😞.

This is typically the only time that you'll see a OAuth token object because you'll generally want to rely on httr2 to manage the tokens for you.
You do by by using `req_oauth_auth_code()`, as described next.
Most APIs provide some way to get information about the current user, so this is typically a good place to test that you've set everything up correctly.

For example, with GitHub we can make a `GET` request to `/user`.
If we do this without setting up authentication, we'll get an error:
...
We can authenticate this request by using `req_oauth_auth_code()`, which takes the same arguments as `oauth_flow_auth_code()`:
...
When you run this code, you'll see something like this, but obviously it will contain information about you, not me.
...
One additional wrinkle is that many APIs don't implement the flow in exactly the same way as the spec.
