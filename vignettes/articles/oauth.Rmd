...
You won't use this when actually performing requests, but it it's the best way to verify that you can get a OAuth token, without having to worry about any other part of the API.
...
You do so by using `req_oauth_auth_code()`, as described next.
...
We can authenticate this request with `req_oauth_auth_code()`, which uses the same arguments as `oauth_flow_auth_code()`:
