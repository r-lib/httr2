...
You've probably used it a bunch without knowing what it's called: it's used when you login to a non-Google website using your Google account, when you give an phone access to your twitter account, or when you login to a streaming app on your smart TV.
...
This vignette builds on the techniques in discussed in `vignette("wrapping-apis")`, so I recommend you read that vignette first.
I'll also assume that you're working in package, since most people want to wrap up an API into a bunch of R functions, but most of the ideas will also apply if you're creating a one off script.

## OAuth basics

OAuth is a broad framework that has many different variants, called **flows**, which makes it hard to provide sweeping generalisations, but the basic idea of OAuth is to create a hierarchy of increasing more specific and shorter lived credentials, so that the impact of a credential being lost is as small as possible.
...
The API gives this permission in the form of an **access token**, which is essentially a random string of numbers and letters.
An access token tends to be very short lived, lasting maybe only days or hours, and is bound to a specified **scope** of access.
...
If you have an access token you can use it to authenticate an API by passing a **bearer token** in the `Authorization` header, which you can do with `req_auth_bearer_token()`.
...
This leads to a hierarchy of credentials from strongest to weakest:

-   user name + password: your code should never touch these!
-   refresh token: it's okay to store this on your computer.
-   access token: you submit this with every request.
...
That will give you access to some sort of developer portal which you can then use to create a new OAuth app (aka a client).
...
1.  httr2 sends opens a browser using the **authorization URL** provided by the API. The URL includes parameters that identify your app and what **scope** of access you're looking for.
2.  The user logs in using their user name and password (hopefully in a password manageer) and approves the request.
...
Each client gets its own subdirectory named using the client `name`, so if you turn caching on, it's particularly important to give you client a good name so that the user can easily tell with package the tokens belong to.
