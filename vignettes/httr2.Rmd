...
We can see exactly this request will send to the server with a dry run:
...
-   The HTTP **method**, which is a verb that tells the server what you want to do.
    Here's its GET, the most common verb, indicating that we want to *get* some data.
    Other verbs include POST, to send data, UPDATE, to update an existing resource, and DELETE, to delete resources.

-   The **path**,which is the url stripped of details that server already knows, i.e. the protocol (http or https), the host (httpbin.org) and the port (not used here).

-   The version of the HTTP protocol.
    This is unimportant for our purposes and it's handled at a lower layer in the stack.

The following lines consistent are the HTTP **headers**, a series of name-value pairs separated by ":".
...
-   The method has changed from GET to POST.
    ...
    If you need to use a different method, call `req_method()`.
...
Different servers need data stored in different formats so httr2 provides a selection of common options.
For example, `req_body_form()` uses the same encoding used when you submit a form from your browser:
...
If you need to data encoded in a different form, you can use `req_body_raw()` to add the data to the body and set the `Content-Type` header.

## Performing a request and fetch the response

To actually perform a request and fetch the response back from the server, call `req_fetch()`:
...
Revert to the httr behaviour with `req_error(req, is_error = ~ FALSE)`.

## Controlling the request process

A number of `req_` functions don't directly affect how the request is sent but affect how the process of submitting the request and handling the response.
I won't give any details here, but you should be aware of the following important functions:
