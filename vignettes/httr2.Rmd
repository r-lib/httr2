...
-   The **path**, which is the URL stripped of details that the server already knows, i.e. the protocol (`http` or `https`), the host (`localhost`), and the port (`R url_parse(test_server())$port`).
...
```{r}
req <- request(test_server()) %>% req_url_path("/json")
