...
to automatically retry failing requests. It's particularly important for
APIs with rate limiting, but can also be useful when dealing with flaky
servers.

By default,
[`req_perform()`](https://httr2.r-lib.org/dev/reference/req_perform.md)
will retry if the response is a 429 ("too many requests", often used for
rate limiting) or 503 ("service unavailable"). If the API you are
wrapping has other transient status codes (or conveys transience with
some other property of the response), you can override the default with
...
[`req_perform()`](https://httr2.r-lib.org/dev/reference/req_perform.md)
will wait a little before trying again:
...
if the API has a rate-limit but doesn't expose the limits in the
response.
