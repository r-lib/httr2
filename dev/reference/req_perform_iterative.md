...
with an [iteration
helper](https://httr2.r-lib.org/dev/reference/iterate_with_offset.md)
and use a [multi-response
handler](https://httr2.r-lib.org/dev/reference/resps_successes.md) to
process the result.
...
- req:
  ...
  The first [request](https://httr2.r-lib.org/dev/reference/request.md)
  to perform.
...
- next_req:
  ...
  (`req`) and returns a
  [request](https://httr2.r-lib.org/dev/reference/request.md) for the
  next page or `NULL` if the iteration should terminate. See below for
  more details.
...
possibly one error object, if `on_error` is `"return"` and one of the
requests errors. If present, the error object will always be the last
element in the list.
...
[`signal_total_pages()`](https://httr2.r-lib.org/dev/reference/signal_total_pages.md),
like this:
